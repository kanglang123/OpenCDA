...
class IntermediateFusionDataset(basedataset.BaseDataset):
    ...
    def __getitem__(self, idx):   
        ...
        #取到数据
        processed_data_dict = OrderedDict()  #建立有序字典
        ...
        for cav_id, cav_content in base_data_dict.items():
            if cav_content['ego']:
                ...
                ego_tarj = cav_content['params']['plan_trajectory'] # 添加轨迹
                # if len(ego_tarj) < 2: # 跳过长度小于2轨迹的数据
                #     print('跳过长度小于2轨迹的数据')
                #     return 
                # ego_tarj_len = len(ego_tarj)
                break
        ...
        for cav_id, selected_cav_base in base_data_dict.items():
            # check if the cav is within the communication range with ego  ！！！！区域划分
            distance = \
            ...
            selected_cav_processed = self.get_item_single_car(
                selected_cav_base,
                ego_lidar_pose,ego_tarj)

            object_stack.append(selected_cav_processed['object_bbx_center'])    #gt值
            ...
            plan_trajectory.append(selected_cav_processed['plan_trajectory'])
            # plan_trajectory_len.append(ego_tarj_len)    
    ...
    def collate_batch_train(self, batch):
        ...
        # used for PriorEncoding for models 先验编码
        velocity = []
        time_delay = []
        infra = []
        plan_trajectory = [] #规划的路径
        pairwise_t_matrix_list = [] # pairwise transformation matrix 位置转换矩阵

        # used for correcting the spatial transformation between delayed timestamp
        # and current timestamp    矫正时间延迟之间的空间位置误差
        spatial_correction_matrix_list = []  #空间校正矩阵列表
        ...
        for i in range(len(batch)):
            ...
            plan_trajectory.append(ego_dict['plan_trajectory'])
        ...
        # plan_trajectory=torch.from_numpy(np.array(plan_trajectory))
    ...
    def collate_batch_test(self, batch):
        assert len(batch) <= 1, "Batch size 1 is required during testing!" #Python assert（断言）用于判断一个表达式，在表达式条件为 false 的时候触发异常。
