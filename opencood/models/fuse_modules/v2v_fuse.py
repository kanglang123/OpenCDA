        ...
        for l in range(self.num_iteration):
            ...
            for b in range(B):
                ...
                for i in range(N):
                    ...
                    # feature_map_viewer1 = batch_node_features[b][0].detach().cpu().numpy() 
                    # # feature_map_viewer1 = np.sum(feature_map_viewer1,axis=0)
                    # plt.imshow(feature_map_viewer1[0])
                    # plt.colorbar()  # 添加颜色条
                    # plt.title('feature_map_viewer1')
                    
                    # (N,C,H,W) 翘曲变换，和时间延迟相关的部分
                    neighbor_feature = warp_affine(batch_node_features[b],
                                                   current_t_matrix,
                                                   (H, W))
                    # fig2 = plt.figure()
                    # # feature_map 自注意力之后的可视化
                    # feature_map_viewer2 = neighbor_feature[b].detach().cpu().numpy() 
                    # # feature_map_viewer2 = np.sum(feature_map_viewer2,axis=0)
                    # plt.imshow(feature_map_viewer2[0]-feature_map_viewer1[0])
                    # plt.colorbar()  # 添加颜色条
                    # plt.title('feature_map_viewer2')
