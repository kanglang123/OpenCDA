...
def visualize_single_sample_dataloader(batch_data,
    ...
    #可视化轨迹 
    plan_trajectory = batch_data['plan_trajectory'][0]
    traj_linset = []
    for i in range(len(plan_trajectory)):
        points = plan_trajectory[i]
        lines = [[j,j+1] for j in range(len(points)-1)]
        colors = [[1, 0, 0] for i in range(len(points)-1)]
        line_set = o3d.geometry.LineSet(
            points=o3d.utility.Vector3dVector(points),
            lines=o3d.utility.Vector2iVector(lines),
            )
        line_set.colors = o3d.utility.Vector3dVector(colors)
        traj_linset.append(line_set)
    

    visualize_elements = [o3d_pcd] + aabbs + traj_linset
